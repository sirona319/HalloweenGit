...
public class StraightPointVecMove : BaseMove
{
    [SerializeField] protected List<Transform> movePointLists = new();
    [SerializeField] protected List<Vector3> movePointListsVec = new();
    ...
    public override void Initialize()
    {
        ...
        if (gameObject.GetComponent<TargetSet>() != null)
        {
            ...
            targetSet.SetPointArrayVec(movePointLists, movePointListsVec);
        }
    ...
    public override void MoveUpdate()
    {
        ...
        float len = Vector3.Distance(transform.position, movePointListsVec[targetNo]);
        ...
        if (len < ENDMOVELEN)
        {
            ...
            else
            {
                ...
                Debug.Log("方向変換");
                direction = (movePointListsVec[targetNo] - transform.position).normalized;
            }
        }
        // Debug.Log("目標へ到着");
